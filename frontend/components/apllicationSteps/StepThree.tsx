...
export default function StepThree({
  ...
  form: Step3Form;
  onChange: (field: keyof Step3Form, value: string) => void;
  onNext?: () => void;
  onBack?: () => void;
  saving?: boolean;
}) {
  const [errors, setErrors] = useState<
    Partial<Record<keyof Step3Form, string>>
  >({});

  const validate = () => {
    const errs: Partial<Record<keyof Step3Form, string>> = {};
    if (!form.firmType)
      errs.firmType = "Please select a firm type (Existing or New).";
    setErrors(errs);
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-32">
          <div className="space-y-2">
            <h2 className="text-3xl font-black tracking-tight">
              Step 3: Deck &amp; Track Record
            </h2>
            <p className="text-gray-500 max-w-2xl">
              Provide your pitch deck and historical performance data for the
              Selection Committee.
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="mt-4 text-xs text-gray-500 flex items-center gap-1.5">
              <Info className="text-blue-600" size={14} /> Your deck will be
              shared securely with the Selection Committee.
            </p>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label
                className={`relative flex cursor-pointer rounded-xl border-2 p-4 transition-all ${form.firmType === "existing" ? "border-blue-600 bg-blue-50" : "border-gray-200 hover:bg-gray-50"}`}
              >
                <input
                  name="firm_type"
                  type="radio"
                ...
                <div className="ml-3 flex flex-col">
                  ...
                  <span className="mt-1 block text-xs text-gray-500">
                    We have at least one closed fund with historical data.
                  </span>
                </div>
              </label>
              <label
                className={`relative flex cursor-pointer rounded-xl border-2 p-4 transition-all ${form.firmType === "new" ? "border-blue-600 bg-blue-50" : "border-gray-200 hover:bg-gray-50"}`}
              >
                <input
                  name="firm_type"
                  type="radio"
                ...
                <div className="ml-3 flex flex-col">
                  ...
                  <span className="mt-1 block text-xs text-gray-500">
                    This is our first fund or we are an emerging manager.
                  </span>
                </div>
              </label>
            </div>
            {errors.firmType && (
              <p className="text-red-500 text-xs mt-2">{errors.firmType}</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <tr className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-4">
                      <input
                        type="number"
                        placeholder="YYYY"
                        className="w-16 bg-transparent border-none p-0 text-sm focus:ring-0"
                      />
                    </td>
                    <td className="px-4 py-4">
                      <input
                        type="text"
                        placeholder="0.0"
                        className="w-16 bg-transparent border-none p-0 text-sm text-right focus:ring-0"
                      />
                    </td>
                    <td className="px-4 py-4">
                      <input
                        type="text"
                        placeholder="0"
                        className="w-12 bg-transparent border-none p-0 text-sm text-right focus:ring-0"
                      />
                    </td>
                    <td className="px-4 py-4">
                      <input
                    ...
                    <td className="px-4 py-4 text-blue-600 font-bold">
                      <input
                        type="text"
                        placeholder="0.0%"
                        className="w-16 bg-transparent border-none p-0 text-sm text-right focus:ring-0 text-blue-600 font-bold"
                      />
                    </td>
            ...
            <div className="p-6 bg-gray-50/50">
              <button
                type="button"
                className="flex items-center gap-2 px-5 py-2.5 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 text-sm font-bold hover:border-blue-600 hover:text-blue-600 transition-all w-full justify-center"
              >
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide">
                Additional performance notes or context
              </label>
              <textarea
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-blue-600 text-white border-t border-white/10 py-5 px-4 md:px-10 z-[60] shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <button
            type="button"
            onClick={onBack}
            className="flex items-center gap-2 px-6 py-2.5 rounded-lg border border-white/30 hover:bg-white/10 text-sm font-bold transition-colors uppercase tracking-wide text-white"
          >
          ...
          <button
            ...
            onClick={handleNext}
            disabled={saving}
            className="flex items-center gap-2 px-8 py-2.5 bg-white rounded-lg text-blue-600 text-sm font-bold hover:bg-white/90 shadow-lg transition-all uppercase tracking-wide disabled:opacity-60"
          >
            {saving ? (
