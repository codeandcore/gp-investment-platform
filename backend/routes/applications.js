...
const { applicationValidators, adminValidators } = require('../middleware/validate');
const {
    getMyApplication,
    saveStep,
    submitApplication,
...
// PUT /api/applications/:id/step/:stepNumber  (auto-save)
router.put(
    '/:id/step/1',
    authenticate,
    authorize('gp'),
    applicationValidators.step1,
    saveStep
);
router.put(
    '/:id/step/2',
...
router.put(
    ...
    authenticate,
    authorize('gp'),
    applicationValidators.step3,
    saveStep
);
router.put('/:id/step/4', authenticate, authorize('gp'), saveStep);
...
router.get('/', authenticate, authorize('admin', 'reviewer'), adminValidators.listApplications, listApplications);
...
router.get('/:id', authenticate, authorize('admin', 'reviewer'), getApplicationDetail);
...
router.patch('/:id/database-status', authenticate, authorize('admin'), updateDatabaseStatus);

// PATCH /api/applications/:id/owner
router.patch('/:id/owner', authenticate, authorize('admin'), adminValidators.changeOwner, changeOwner);

// POST /api/applications/:id/notes
router.post('/:id/notes', authenticate, authorize('admin'), adminValidators.addNote, addNote);
